...
{
	if(verbose_debug) Genode::log("Cpu_thread::\033[33m", __func__, "\033[0m()");
...
{
	if(verbose_debug) Genode::log("Cpu_thread::\033[33m", __func__, "\033[0m(ip=",
...
{
	if(verbose_debug) Genode::log("Cpu_thread::\033[33m", __func__, "\033[0m(state<kcap=",
...
{
	if(verbose_debug) Genode::log("Cpu_thread::\033[33m", __func__, "\033[0m(sigh=", handler, ")");
	_parent_cpu_thread.exception_sigh(handler);
}

void Cpu_thread_component::single_step(bool enabled)
{
	if(verbose_debug) Genode::log("Cpu_thread::\033[33m", __func__, "\033[0m(", enabled, ")");
	_parent_cpu_thread.single_step(enabled);
}

void Cpu_thread_component::affinity(Genode::Affinity::Location location)
{
	if(verbose_debug) Genode::log("Cpu_thread::\033[33m", __func__, "\033[0m(loc<pos=",
