...
Genode::Capability<Genode::Signal_source> Pd_session_component::alloc_signal_source()
{
	...
	Genode::Lock::Guard guard(_parent_state.source_lock);
	_parent_state.source_objs.insert(new_ss_info);
...
void Pd_session_component::free_signal_source(Genode::Capability<Genode::Signal_source> cap)
{
	...
	// Find list element
	Genode::Lock::Guard guard(_parent_state.source_lock);
	Signal_source_info *ss_info = _parent_state.source_objs.first();
	...
	if(ss_info)
	{
		...
		_parent_state.source_objs.remove(ss_info);
		...
		unsigned long imprint)
{
	...
	Genode::Lock::Guard guard(_parent_state.context_lock);
	_parent_state.context_objs.insert(new_sc_info);
...
void Pd_session_component::free_context(Genode::Signal_context_capability cap)
{
	...
	// Find list element
	Genode::Lock::Guard guard(_parent_state.context_lock);
	Signal_context_info *sc_info = _parent_state.context_objs.first();
	...
	{
		// Remove and destroy list element
		_parent_state.context_objs.remove(sc_info);
...
Genode::Native_capability Pd_session_component::alloc_rpc_cap(Genode::Native_capability ep)
{
	...
	Genode::Lock::Guard guard(_parent_state.native_cap_lock);
	_parent_state.native_cap_objs.insert(new_nc_info);
...
void Pd_session_component::free_rpc_cap(Genode::Native_capability cap)
{
	...
	// Find list element
	Genode::Lock::Guard guard(_parent_state.native_cap_lock);
	Native_capability_info *nc_info = _parent_state.native_cap_objs.first();
	...
	{
		// Remove and destroy list element
		_parent_state.native_cap_objs.remove(nc_info);
