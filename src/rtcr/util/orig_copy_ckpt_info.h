/*
 * \brief  List element to store a mapping of original dataspace to copy dataspace
 * \author Denis Huber
 * \date   2016-11-23
 */

#ifndef _RTCR_ORIG_COPY_INFO_H_
#define _RTCR_ORIG_COPY_INFO_H_
...
namespace Rtcr {
	struct Orig_copy_info;
}


struct Rtcr::Orig_copy_info : Genode::List<Orig_copy_info>::Element
{
	...
	Orig_copy_info(Genode::Dataspace_capability orig_ds_cap, Genode::Ram_dataspace_capability copy_ds_cap,
	...
	Orig_copy_info *find_by_orig_badge(Genode::uint16_t badge)
	{
		if(badge == orig_ds_cap.local_name())
			return this;
		Orig_copy_info *info = next();
		return info ? info->find_by_orig_badge(badge) : 0;
	}

	Orig_copy_info *find_by_copy_badge(Genode::uint16_t badge)
	{
