...
Region_map_component &Rm_session_component::_create(Genode::size_t size)
{
	...
	Region_map_component *new_region_map =
			new (_md_alloc) Region_map_component(_md_alloc, parent_cap, size, "custom", _bootstrap_phase, _ep);
...
Rm_session_component::Rm_session_component(Genode::Env &env, Genode::Allocator &md_alloc, Genode::Rpc_entrypoint &ep,
...
Rm_session_component *Rm_root::_create_session(const char *args)
{
	...
	Rm_session_component *new_session =
			new (md_alloc()) Rm_session_component(_env, _md_alloc, _ep.rpc_ep(), readjusted_args, _bootstrap_phase);
